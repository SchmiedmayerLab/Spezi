...
extension Diagnostic {
    init<S: SyntaxProtocol>(
        ...
        self.init(node: Syntax(syntax), message: SpeziSchedulerDiagnostic(message: message, domain: domain, id: id, severity: severity))
    }
}
...
extension DiagnosticsError {
    init<S: SyntaxProtocol>(
        ...
        self.init(diagnostics: [
            Diagnostic(node: Syntax(syntax), message: SpeziSchedulerDiagnostic(message: message, domain: domain, id: id, severity: severity))
        ])
    }
}
